...
namespace ui {
	LayerWithBorderHorizontal::LayerWithBorderHorizontal(std::vector<IScalable *> objects, std::vector<float> bounds, sf::Vector2f minSize) :
		ILayer(minSize), LayoutWithObjectsArray(std::move(objects)), boundsHorizontal(std::move(bounds)) {
	...
	ILayer(minSize), LayoutWithObjectsArray(std::move(objects)), boundsHorizontal(this->objects.size() + 1, 1.0f) {
	...
	LayerWithBorderHorizontal::LayerWithBorderHorizontal(IScalable *first, IScalable *second, float bound, sf::Vector2f minSize) :
		ILayer(minSize), LayoutWithObjectsArray({first, second}), boundsHorizontal({0.f, bound, 1.f}) {}
