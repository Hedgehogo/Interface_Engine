...
namespace ui {
	Sprite::Sprite(sf::Texture &texture, sf::IntRect rect, sf::Vector2f minSize) :
		IDrawable(), sprite(texture, rect), minimumSize(std::max(minSize.x, 1.f), std::max(minSize.y, 1.f)) {
		sprite.setTextureRect(rect);
	}
	
	Sprite::Sprite(sf::Texture &texture, sf::Vector2f minSize) :
		IDrawable(), sprite(texture), minimumSize(std::max(minSize.x, 1.f), std::max(minSize.y, 1.f)) {
	}
