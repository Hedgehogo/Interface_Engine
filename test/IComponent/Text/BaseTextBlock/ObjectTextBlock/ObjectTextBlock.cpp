...
TEST(IComponent_Text, ObjectTextBlock) {
	...
	font.loadFromFile("../../example-resources/msyh.ttc");
	{
		...
		ASSERT_TRUE(object_text_block.handle_event(ie::Event::Touch(std::numeric_limits<size_t>::max(), {})));
	...
	}
	{
		...
		ASSERT_FALSE(object_text_block.handle_event(ie::Event::Touch(std::numeric_limits<size_t>::max(), {})));
