...
namespace ui {
	...
	LayerWithBorder::LayerWithBorder(std::vector<std::vector<IScalable *>> objects, std::vector<float> boundsHorizontal, std::vector<float> boundsVertical, sf::Vector2f minSize) :
		Layer(minSize), objects(std::move(objects)), boundsVertical(std::move(boundsVertical)), boundsHorizontal(std::move(boundsHorizontal)) {
	...
	LayerWithBorder::LayerWithBorder(std::vector<std::vector<IScalable *>> objects, sf::Vector2f minSize) :
		Layer(minSize), objects(std::move(objects)), boundsHorizontal(this->objects.size() + 1, 1.0f), boundsVertical(this->objects[0].size() + 1, 1.0f) {
	...
	LayerWithBorder *LayerWithBorder::copy() {
		...
		if(count.x != 0) count.y = objects[0].size();
		std::vector<std::vector<IScalable *>> newObjects(count.x, std::vector<IScalable *>(count.y, nullptr));
