...
namespace ui {
	VideoFromTexture::VideoFromTexture(sf::Texture &texture, PSCoefficient viewingProgress, sf::IntRect rect, int countFrame, sf::Vector2i offset) :
		...
		if (rect.height == 0) this->rect.height = textureSize.y - rect.top;
		if (rect.width == 0) this->rect.width = this->rect.height;

		if (countFrame == 0) this->countFrame = this->textureSize.x / this->rect.width;
