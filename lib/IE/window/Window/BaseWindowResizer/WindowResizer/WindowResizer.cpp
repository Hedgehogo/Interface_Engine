...
namespace ie {
	WindowResizer::WindowResizer(int internal_border_size, int external_border_size, Key key) : internal_border_size_(internal_border_size), external_border_size_(external_border_size), key_(key){
	}
	...
	void WindowResizer::set_size() {
		sf::Vector2i size{borders_.right - borders_.left, borders_.down - borders_.up};
		...
		sf::Vector2f min_size{window_->get_min_size()};
	...
	void WindowResizer::set_borders() {
		sf::Vector2i window_position{window_->get_window().getPosition()};
		sf::Vector2u window_size{window_->get_window().getSize()};
		borders_ = {
			window_position.y,
	...
	bool WindowResizer::update(sf::Vector2i mouse_position) {
		if(KeyHandler::is_key_pressed(key_)) {
			if(current_border_.x || current_border_.y) {
				...
				sf::Vector2i window_size{window_->get_window().getSize()};
				set_borders();
				
				if(mouse_position.x >= -external_border_size_ && mouse_position.y >= -external_border_size_ && mouse_position.x <= window_size.x + external_border_size_ && mouse_position.y <= window_size.y + external_border_size_) {
