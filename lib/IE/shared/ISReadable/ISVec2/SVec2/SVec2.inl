...
namespace ie::make_system {
	...
	template<typename Value_, typename _>
	auto BasicSVec2<Value_, _>::make(SInitInfo init_info) -> rttb::Dyn {
	...
	template<typename Value_>
	auto BasicSMVec2<Value_>::make(SInitInfo init_info) -> rttb::Dyn {
		return rttb::Dyn{ie::BasicSMVec2<Value_>{std::move(*this), SInitInfo{init_info}}};
	}
}

template<typename Value_>
auto ieml::Decode<char, ie::make_system::BasicSVec2<Value_> >::decode(const ieml::Node& node) -> orl::Option<ie::make_system::BasicSVec2<Value_> > {
...
template<typename Value_>
auto ieml::Decode<char, ie::make_system::BasicSMVec2<Value_> >::decode(const ieml::Node& node) -> orl::Option<ie::make_system::BasicSMVec2<Value_> > {
...
namespace ie {
	...
	template<typename Value_, typename _>
	auto BasicSVec2<Value_, _>::get_x() const -> Value_& {
		return x_.get();
	}
	
	template<typename Value_, typename _>
	auto BasicSVec2<Value_, _>::get_y() const -> Value_& {
		return y_.get();
	}
	...
	template<typename Value_, typename _>
	auto BasicSVec2<Value_, _>::delete_read_fn(ReadFn& read_fn) -> bool {
	...
	template<typename Value_, typename _>
	auto BasicSVec2<Value_, _>::reset() -> void {
		if(reset_) {
			reset_ = false;
			auto value{get()};
	...
	template<typename Value_>
	auto BasicSMVec2<Value_>::get_x() const -> ToMutable<Value_>& {
		return this->x_.get();
	}
	
	template<typename Value_>
	auto BasicSMVec2<Value_>::get_y() const -> ToMutable<Value_>& {
		return this->y_.get();
	}
	...
	template<typename Value_>
	auto BasicSMVec2<Value_>::set(T value) -> void {
		this->reset_ = false;
	...
	auto Determine<make_system::BasicSMVec2<ISRanged<T_> > >::determine(ieml::Node const& node) -> bool {
