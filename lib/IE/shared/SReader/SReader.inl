...
namespace ie {
	...
	SReader<Value_, _>::SReader(Value_& readable, ReadFn&& read_fn) :
		readable_(&readable), read_fn_(&static_cast<ISReadable<T>&>(readable).add_read_fn(std::move(read_fn))) {
	}
	
	template<typename Value_, typename _>
	SReader<Value_, _>::SReader(const SReader& other) :
		readable_(other.readable_), read_fn_(&static_cast<ISReadable<T>&>(*other.readable_).add_read_fn(ReadFn{*other.read_fn_})) {
	}
	...
	SReader<Value_, _>::~SReader() {
		if(readable_) {
			static_cast<ISReadable<T>&>(*readable_).delete_read_fn(*read_fn_);
		}
	}
}
