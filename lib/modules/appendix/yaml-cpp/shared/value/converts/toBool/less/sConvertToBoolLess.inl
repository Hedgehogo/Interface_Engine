...
namespace ui {
	template<typename T>
	SConvertToBoolLess<T>::SConvertToBoolLess(PISValue<T> fromValue, T compared) : BaseSConvertToBoolComparison<T>(fromValue, convert(fromValue->getValue(), compared), compared) {
	}
	...
	template<typename T>
	bool convertPointer(const YAML::Node &node, SConvertToBoolLess<T> *&sConvertToBoolLess) {
